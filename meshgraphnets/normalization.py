...
class Normalizer(snt.AbstractModule):
  ...
  def __init__(self, size, max_accumulations=10**6, std_epsilon=1e-8,
    ...
    with self._enter_variable_scope():
      self._acc_count = tf.Variable(0, dtype=tf.float16, trainable=False)
      self._num_accumulations = tf.Variable(0, dtype=tf.float16,
                                            trainable=False)
      self._acc_sum = tf.Variable(tf.zeros(size, tf.float16), trainable=False)
      self._acc_sum_squared = tf.Variable(tf.zeros(size, tf.float16),
                                          trainable=False)
  ...
  def _accumulate(self, batched_data):
    ...
    count = tf.cast(tf.shape(batched_data)[0], tf.float16)
