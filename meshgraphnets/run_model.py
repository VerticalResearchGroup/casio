...
FLAGS = flags.FLAGS
flags.DEFINE_enum('execstyle', 'none', ['none', 'tfprofile', 'nsight', 'createchkpoint'],
                  'Execution style')
...
PARAMETERS = {
    ...
    'cloth': dict(noise=0.003, gamma=0.1, field='world_pos', history=True,
                  size=3, batch=2, model=cloth_model, evaluator=cloth_eval)
}
...
def learner(model, params):
  ...
  if FLAGS.execstyle == 'createchkpoint':
      checkpoint_freq = 600
  else:
  ...
  with tf.train.MonitoredTrainingSession(
    ...
    if FLAGS.execstyle == 'createchkpoint':
      ...
      def roi(run_options=None, run_metadata=None):
        sess.run([train_op, global_step, loss_op], options=run_options, run_metadata=run_metadata)

      benchmark_wrapper('meshgraphnets', roi, sess)

    exit(0)

    if FLAGS.execstyle == 'tfprofile':
    ...
    while not sess.should_stop():
      if FLAGS.execstyle == 'tfprofile':
        if (i != 0):
          ...
          _, step, loss = sess.run([train_op, global_step, loss_op], options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),run_metadata=run_meta)
          end_time = time.time()
        else:
          start_time = time.time()
          _, step, loss = sess.run([train_op, global_step, loss_op])
