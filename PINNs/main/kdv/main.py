...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, u1, layers, dt, lb, ub, q):
        ...
        self.x0 = x0.astype(np.float16)
        self.x1 = x1.astype(np.float16)

        self.u0 = u0.astype(np.float16)
        self.u1 = u1.astype(np.float16)
        ...
        self.lambda_1 = tf.Variable([0.0], dtype=tf.float16)
        self.lambda_2 = tf.Variable([-6.0], dtype=tf.float16)

        # Load IRK weights
        tmp = np.float16(np.loadtxt('../../Utilities/IRK_weights/Butcher_IRK%d.txt' % (q), ndmin = 2))
        ...
        self.x1_tf = tf.placeholder(tf.float16, shape=(None, self.x1.shape[1]))
        self.u0_tf = tf.placeholder(tf.float16, shape=(None, self.u0.shape[1]))
        self.u1_tf = tf.placeholder(tf.float16, shape=(None, self.u1.shape[1]))
        self.dummy_x0_tf = tf.placeholder(tf.float16, shape=(None, self.q)) # dummy variable for fwd_gradients
        self.dummy_x1_tf = tf.placeholder(tf.float16, shape=(None, self.q)) # dummy variable for fwd_gradients
    ...
    def train(self, nIter):
        ...
        benchmark_wrapper('pinn-kdv', roi, self.sess)
        exit(0)
