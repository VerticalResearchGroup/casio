...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, layers, dt, lb, ub, q):
        ...
        self.IRK_weights = np.reshape(tmp[0:q**2+q], (q+1,q)).astype(np.float16)
        self.IRK_times = tmp[q**2+q:].astype(np.float16)
        ...
        self.dummy_x1_tf = tf.placeholder(tf.float16, shape=(None, self.q+1)) # dummy variable for fwd_gradients
