...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, u, v, layers):
        ...
        X = np.concatenate([x, y, t], 1).astype(np.float16)
        ...
        self.x = X[:,0:1].astype(np.float16)
        self.y = X[:,1:2].astype(np.float16)
        self.t = X[:,2:3].astype(np.float16)

        self.u = u.astype(np.float16)
        self.v = v.astype(np.float16)
        ...
        self.lambda_2 = tf.Variable([0.0], dtype=tf.float16)

        # tf placeholders and graph
        self.sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
                                                     log_device_placement=False))

        self.x_tf = tf.placeholder(tf.float16, shape=[None, self.x.shape[1]])
        self.y_tf = tf.placeholder(tf.float16, shape=[None, self.y.shape[1]])
        self.t_tf = tf.placeholder(tf.float16, shape=[None, self.t.shape[1]])

        self.u_tf = tf.placeholder(tf.float16, shape=[None, self.u.shape[1]])
        self.v_tf = tf.placeholder(tf.float16, shape=[None, self.v.shape[1]])
    ...
    def train(self, nIter):
        ...
        benchmark_wrapper('pinn-navier-stokes', roi, self.sess)
        exit(0)
