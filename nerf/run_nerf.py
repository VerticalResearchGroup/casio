...
def batchify(fn, chunk):
    ...
    def ret(inputs):
        inputs = tf.cast(inputs,dtype=tf.dtypes.float16)
        temp = tf.concat([fn(inputs[i:i+chunk]) for i in range(0, inputs.shape[0], chunk)], 0)
        return tf.cast(temp,dtype=tf.dtypes.float32)
    return ret
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    #end = time.time()
    #print('DNN Forward Pass Time: ',(end-start))
