...
class TabNet(object):
  ...
  def encoder(self, data, reuse, is_training):
    ...
    with tf.variable_scope("Encoder", reuse=reuse):
      ...
      features_shadow = tf.feature_column.input_layer(data, self.columns)
      features = tf.identity(features_shadow)
      features = tf.cast(features, tf.float16)
      print('****************************')
      print(features_shadow)
      print('****************************')
      features = tf.layers.batch_normalization(
          features,
	  beta_initializer=tf.zeros_initializer(dtype=tf.dtypes.float16),
	  gamma_initializer = tf.ones_initializer(dtype=tf.dtypes.float16),
	  moving_mean_initializer=tf.zeros_initializer(dtype=tf.dtypes.float16),
	  moving_variance_initializer=tf.ones_initializer(dtype=tf.dtypes.float16),
      ...
      output_aggregated = tf.zeros([batch_size, self.output_dim],dtype=tf.dtypes.float16)
      masked_features = features
      mask_values = tf.zeros([batch_size, self.num_features],dtype=tf.dtypes.float16)
      aggregated_mask_values = tf.zeros([batch_size, self.num_features],dtype=tf.dtypes.float16)
      complemantary_aggregated_mask_values = tf.ones(
          [batch_size, self.num_features],dtype=tf.dtypes.float16)
      total_entropy = 0
      ...
      for ni in range(self.num_decision_steps):
        ...
        if ni < self.num_decision_steps - 1:
          ...
          mask_values = tf.cast(tf.contrib.sparsemax.sparsemax(tf.cast(mask_values, tf.float32)), tf.float16)
