#!/bin/bash
APPNAME=unet
INFER_CMD="python -m unet.infer"
TRAIN_CMD="python -m unet.train"
...
APP=$APPNAME-infer DT=$INFER_DT BS=$I_SMALL_BS NW=1 NI=1  $CASIO/utils/run_ncu_min.sh $INFER_CMD
APP=$APPNAME-infer DT=$INFER_DT BS=$I_LARGE_BS NW=1 NI=1  $CASIO/utils/run_ncu_min.sh $INFER_CMD
APP=$APPNAME-train DT=$TRAIN_DT BS=$T_SMALL_BS NW=1 NI=1  $CASIO/utils/run_ncu_min.sh $TRAIN_CMD
APP=$APPNAME-train DT=$TRAIN_DT BS=$T_LARGE_BS NW=1 NI=1  $CASIO/utils/run_ncu_min.sh $TRAIN_CMD
